...
plt.plot(*Xt_pca[:, :2].T, "o", label="OU", alpha=0.7, markersize=2)
plt.plot(*params_pca[:, :2].T, "+", label="SGD", alpha=0.7, markersize=2)
plt.legend(["OU", "SGD"], title="PCA Realization of", frameon=True)
plt.xlabel(r"$\theta^{PCA 1}$")
plt.ylabel(r"$\theta^{PCA 2}$")
plt.title(r"PCA of $\theta$ for a realization of SGD an OU on a toy dataset (task3)")
